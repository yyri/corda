...
interface ServiceHub : ServicesForResolution {
     ...
     */
    fun signInitialTransaction(builder: TransactionBuilder, publicKey: PublicKey, transactionSignatureMeta: TransactionSignatureMeta): SignedTransaction {
        val merkleRootWithMeta = MerkleRootWithMeta(builder.toWireTransaction().id, transactionSignatureMeta)
        val sig = keyManagementService.sign(merkleRootWithMeta, publicKey)
        builder.addSignatureUnchecked(sig)
        return builder.toSignedTransaction(false)
    }

    /** The default [TransactionSignatureMeta] is used*/
    fun signInitialTransaction(builder: TransactionBuilder, publicKey: PublicKey) =
            signInitialTransaction(builder, publicKey, TransactionSignatureMeta(myInfo.platformVersion))
     ...
     */
    fun createSignature(signedTransaction: SignedTransaction, publicKey: PublicKey) = createSignature(signedTransaction, publicKey, TransactionSignatureMeta(myInfo.platformVersion))

    fun createSignature(signedTransaction: SignedTransaction, publicKey: PublicKey, transactionSignatureMeta: TransactionSignatureMeta): TransactionSignature {
        val merkleRootWithMeta = MerkleRootWithMeta(signedTransaction.id, transactionSignatureMeta)
        return keyManagementService.sign(merkleRootWithMeta, publicKey)
    }
