...
class TransactionKeyFlow(val otherSide: Party,
                         ...
                         override val progressTracker: ProgressTracker) : FlowLogic<LinkedHashMap<Party, AnonymousPartyAndCertificate>>() {
    ...
    companion object {
        ...
        fun validateIdentity(otherSide: Party, anonymousOtherSide: AnonymousPartyAndCertificate): AnonymousPartyAndCertificate {
    ...
    @Suspendable
    override fun call(): LinkedHashMap<Party, AnonymousPartyAndCertificate> {
        ...
        val identities = LinkedHashMap<Party, AnonymousPartyAndCertificate>()
        if (otherSide == serviceHub.myInfo.legalIdentity) {
            identities.put(otherSide, legalIdentityAnonymous)
        } else {
            val otherSideAnonymous = sendAndReceive<AnonymousPartyAndCertificate>(otherSide, legalIdentityAnonymous).unwrap { validateIdentity(otherSide, it) }
