...
class FlowFrameworkTests {
    ...
    fun `exception while fiber suspended`() {
        node2.registerFlowFactory(ReceiveFlow::class.java) { SendFlow("Hello", it) }
    ...
    fun `flow restarted just after receiving payload`() {
        node2.registerFlowFactory(SendFlow::class.java) { ReceiveFlow(it).nonTerminating() }
    ...
    fun `flow loaded from checkpoint will respond to messages from before start`() {
        node1.registerFlowFactory(ReceiveFlow::class.java) { SendFlow("Hello", it) }
    ...
    fun `flow with send will resend on interrupted restart`() {
        ...
        val secondFlow = node3.registerFlowFactoryAndGetFirstInitiatedFlow(PingPongFlow::class) { PingPongFlow(it, payload2) }
        net.runNetwork()
    ...
    fun `sending to multiple parties`() {
        ...
        net.runNetwork()
        node2.registerFlowFactory(SendFlow::class.java) { ReceiveFlow(it).nonTerminating() }
        node3.registerFlowFactory(SendFlow::class.java) { ReceiveFlow(it).nonTerminating() }
    ...
    fun `receiving from multiple parties`() {
        ...
        node2.registerFlowFactory(ReceiveFlow::class.java) { SendFlow(node2Payload, it) }
        node3.registerFlowFactory(ReceiveFlow::class.java) { SendFlow(node3Payload, it) }
    ...
    fun `both sides do a send as their first IO request`() {
        node2.registerFlowFactory(PingPongFlow::class.java) { PingPongFlow(it, 20L) }
    ...
    fun `other side ends before doing expected send`() {
        node2.registerFlowFactory(ReceiveFlow::class.java) { NoOpFlow() }
    ...
    fun `non-FlowException thrown on other side`() {
        val erroringFlowFuture = node2.registerFlowFactoryAndGetFirstInitiatedFlow(ReceiveFlow::class) {
    ...
    fun `FlowException thrown on other side`() {
        val erroringFlow = node2.registerFlowFactoryAndGetFirstInitiatedFlow(ReceiveFlow::class) {
    ...
    fun `FlowException propagated in invocation chain`() {
        ...
        node3.registerFlowFactory(ReceiveFlow::class.java) { ExceptionFlow { MyFlowException("Chain") } }
        node2.registerFlowFactory(ReceiveFlow::class.java) { ReceiveFlow(node3.info.legalIdentity) }
    ...
    fun `FlowException thrown and there is a 3rd unrelated party flow`() {
        ...
        val node2Fiber = node2
                .registerFlowFactoryAndGetFirstInitiatedFlow(ReceiveFlow::class) { SendAndReceiveFlow(it, "Hello") }
                .map { it.stateMachine }
        node3.registerFlowFactory(ReceiveFlow::class.java) { ExceptionFlow { MyFlowException("Nothing useful") } }
    ...
    fun `retry subFlow due to receiving FlowException`() {
        ...
        node2.registerFlowFactory(AskForExceptionFlow::class.java) { ConditionalExceptionFlow(it, "Hello") }
    ...
    fun `serialisation issue in counterparty`() {
        node2.registerFlowFactory(ReceiveFlow::class.java) { SendFlow(NonSerialisableData(1), it) }
    ...
    fun `FlowException has non-serialisable object`() {
        node2.registerFlowFactory(ReceiveFlow::class.java) {
    ...
    fun `wait for transaction`() {
        ...
        val committerFiber = node1.registerFlowFactoryAndGetFirstInitiatedFlow(WaitingFlows.Waiter::class) {
    ...
    fun `committer throws exception before calling the finality flow`() {
        ...
        node1.registerFlowFactory(WaitingFlows.Waiter::class.java) {
    ...
    fun `customised client flow`() {
        val receiveFlowFuture = node2.registerFlowFactoryAndGetFirstInitiatedFlow(SendFlow::class) { ReceiveFlow(it) }
    ...
    fun `unsupported new flow version`() {
        // We can't use registerFlowFactory as it registers with the version on the flow's annotation. We could
        // expose a method that lets us specify the version you want to register with but that's only useful for a test
        // like this, and not useful as a general test method accessible to flow devs
        @Suppress("UNCHECKED_CAST")
        val node2FlowFactories = ReflectionUtils.getFieldVal(node2, "flowFactories") as MutableMap<Class<out FlowLogic<*>>, InitiatedFlowInfo>
        node2FlowFactories[UpgradedFlow::class.java] = InitiatedFlowInfo.CorDapp(version = 1, factory = { SendFlow("Hello", it) })
    ...
    fun `single inlined sub-flow`() {
        node2.registerFlowFactory(SendAndReceiveFlow::class.java, ::SingleInlinedSubFlow)
    ...
    fun `double inlined sub-flow`() {
        node2.registerFlowFactory(SendAndReceiveFlow::class.java, ::DoubleInlinedSubFlow)
        val result = node1.services.startFlow(SendAndReceiveFlow(node2.info.legalIdentity, "Hello")).resultFuture
        net.runNetwork()
        assertThat(result.getOrThrow()).isEqualTo("HelloHello")
    }
    ...
    inline fun <reified P : FlowLogic<*>> MockNode.registerFlowFactoryAndGetFirstInitiatedFlow(
        initiatingFlowClass: KClass<out FlowLogic<*>>,
        noinline flowFactory: (Party) -> P): ListenableFuture<P>
    {
        val firstInitiatedFlow = smm.changes
            .filter { it is StateMachineManager.Change.Add && it.logic is P }
            .map { it.logic as P }
            .toFuture()
        registerFlowFactory(initiatingFlowClass.java, flowFactory)
        return firstInitiatedFlow
    }
