...
abstract class AbstractNode(open val configuration: NodeConfiguration,
    ...
    val services = object : ServiceHubInternal() {
        ...
        override fun registerServiceFlow(clientFlowClass: Class<out FlowLogic<*>>, serviceFlowFactory: (Party) -> FlowLogic<*>) {
            ...
            log.info("Registering service flow for ${clientFlowClass.name} version $version")
            serviceFlowFactories[clientFlowClass] = ServiceFlowInfo.CorDapp(version, serviceFlowFactory)
        }
     ...
     * [FlowVersion], core flows have the same version as the node's platform version. To cater for backwards compatiblity
...
sealed class ServiceFlowInfo {
    class Core(val factory: (Party, Int) -> FlowLogic<*>) : ServiceFlowInfo()
    class CorDapp(val version: Int, val factory: (Party) -> FlowLogic<*>) : ServiceFlowInfo()
}
