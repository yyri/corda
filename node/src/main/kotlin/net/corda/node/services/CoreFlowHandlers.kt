...
class IdentitySyncHandler(val otherSide: Party) : FlowLogic<Unit>() {
    ...
    override fun call(): Unit {
        ...
        val unknownIdentities = allIdentities.filter { serviceHub.identityService.anonymousFromKey(it.owningKey) == null }
        val missingIdentities: List<AnonymousPartyAndPath> = sendAndReceive<List<AnonymousPartyAndPath>>(otherSide, unknownIdentities).unwrap { it }
        missingIdentities.forEach { identity ->
            serviceHub.identityService.verifyAndRegisterAnonymousIdentity(identity)
        }
