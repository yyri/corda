...
class StateMachineManager(val serviceHub: ServiceHubInternal,
    ...
    private fun onSessionInit(sessionInit: SessionInit, receivedMessage: ReceivedMessage, sender: Party) {
        ...
        val initiatedFlowInfo = serviceHub.getFlowFactory(sessionInit.initiatingFlowClass)
        if (initiatedFlowInfo == null) {
            logger.warn("${sessionInit.initiatingFlowClass} has not been registered: $sessionInit")
            sendSessionReject("Don't know ${sessionInit.initiatingFlowClass.name}")
            return
        }

        val session = try {
            val flow = when (initiatedFlowInfo) {
                is InitiatedFlowInfo.CorDapp -> {
                    // TODO Add support for multiple versions of the same flow when CorDapps are loaded in separate class loaders
                    if (sessionInit.flowVerison != initiatedFlowInfo.version) {
                        logger.warn("Version mismatch - ${sessionInit.initiatingFlowClass} is only registered for version " +
                                "${initiatedFlowInfo.version}: $sessionInit")
                        sendSessionReject("Version not supported")
                        return
                    }
                    initiatedFlowInfo.factory(sender)
                }
                is InitiatedFlowInfo.Core -> initiatedFlowInfo.factory(sender, receivedMessage.platformVersion)
            }
