...
class StateMachineManager(val serviceHub: ServiceHubInternal,
    ...
    private val serializationContext = SerializeAsTokenContext(tokenizableServices, quasarKryoPool)
        ...
        val session = try {
            val flow = when (serviceFlowInfo) {
                is ServiceFlowInfo.CorDapp -> {
                    // TODO Add support for multiple versions of the same flow when class loading is enabled
